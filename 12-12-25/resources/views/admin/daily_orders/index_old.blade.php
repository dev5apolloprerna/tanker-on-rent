                ...
                <tbody>
                  ...
                  @php
                    ...
                    <tr>
                      ...
                      <button
                        type="button"
                        class="btn btn-sm btn-outline-info js-order-payments"
                        title="View Payments"
...
<div class="modal fade" id="customerPaymentsModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="custPayTitle">Payments</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>

      <div class="modal-body">
        <form class="row g-2 align-items-end mb-3" id="custPayFilter">
          <div class="col-sm-4">
            <label class="form-label small mb-1">From</label>
            <input type="date" class="form-control form-control-sm" id="custPayFrom">
          </div>
          <div class="col-sm-4">
            <label class="form-label small mb-1">To</label>
            <input type="date" class="form-control form-control-sm" id="custPayTo">
          </div>
          <div class="col-sm-4">
            <button class="btn btn-sm btn-primary" id="custPaySearch" type="submit">Search</button>
            <button class="btn btn-sm btn-light" type="button" id="custPayReset">Reset</button>
          </div>
        </form>
...
(function(){
  const modalEl   = document.getElementById('customerPaymentsModal');
  const modal     = new bootstrap.Modal(modalEl);
  const titleEl   = document.getElementById('custPayTitle');
  const bodyEl    = document.getElementById('custPayBody');
  const fromEl    = document.getElementById('custPayFrom');
  const toEl      = document.getElementById('custPayTo');
  const filterEl  = document.getElementById('custPayFilter');
  ...
  async function loadPayments() {
    ...
    const url = "{{ route('customers.payments', ':id') }}"
      .replace(':id', encodeURIComponent(currentCustomerId))
  ...
  document.querySelectorAll('.js-cust-payments').forEach(btn => {
    btn.addEventListener('click', () => {
      currentCustomerId = btn.dataset.customerId;
      currentCustomerName = btn.dataset.customerName || 'Customer';
      titleEl.textContent = `${currentCustomerName} — Payments`;
      initRange();
      modal.show();
      loadPayments();
    });
